...
const parser = StructuredOutputParser.fromZodSchema(
    z.object({
        ...
        suggestions: z.array(z.object({
            ...
            name: z.string().describe(`
                The name of the cubic bezier function, in a professional way and removing any
                obvious names, all in lower case, don't repeat names between functions if they are different
            `),
            description: z.string().describe('The description of the name of the cubic bezier function'),
            title: z.string().describe(`The name of the cubic bezier function, that will be displayed as the title`),
...
const getPrompt = async (bezierFunction: string) => {
    ...
    const prompt = new PromptTemplate({
        template: `You are a professional design engineer (Animations in the web),
            ...
            with a name for it (base yourself on already existing bezier functions in CSS for example, 
            try to make it professional and unique, and use technical vocabulary)
            , and also a description explaining why the name. Follow the instructions and format 
            your response to match the format instructions.\n{format_instructions}\n{entry}`,
